...
import { OrderDetailsProps } from '../profile.types';
...
import FinishOrderDeliveryButton from '../ui/buttons/finish-order-delivery-button/FinishOrderDeliveryButton';
import IOrder from 'src/redux/models/IOrder';
import OpenAddingReviewButton from '../ui/buttons/open-adding-review-button/OpenAddingReviewButton';
...
const OrderDetails = ({currentUser, order, onOpenAddingOrderReview, onOrderPlaced, onOrderDeliveryFinished} : OrderDetailsProps) => {
    ...
    const renderUserDetails = useCallback((currentUser: IUser) => {
                    ...
                    <tr className='order__details__tr'>
                        ...
                        <td>
                            <div className='order__details__courier__wrapper'>
                                <div className='order__details__courier__image__wrapper'>
                                    <img className="order__details__courier__image" src={order.courier.imageUrl} alt="image"/>
                                </div>
    ...
    const renderDestinationAddress = useCallback((order: IOrder) => {
        return (
            <tr className='order__details__tr'>
                <td>
    ...
    const renderDeliveryAcceptedAt = useCallback((order: IOrder) => {
    ...
    const renderDeliveringInformation = useCallback((order: IOrder) => {
    ...
    const renderDeliveredInformation = useCallback((order: IOrder) => {
    ...
    const renderCourierFinishButton = useCallback((currentUser: IUser, order: IOrder) => {
    ...
    const renderReview = useCallback((order: IOrder) => {
        if (currentUser.role === 'customer' && order.status === 'delivered') {

            if (!order.review && onOpenAddingOrderReview) {
                return (
                    <tr className='order__details__tr'>
                        <td colSpan={2}>
                            <OpenAddingReviewButton isReviewAdded={Boolean(order.review)} onOpen={onOpenAddingOrderReview}/>
                        </td>
                    </tr>
                )
            }
                    ...
                    <tr className='order__details__tr'>
                        <td colSpan={2}>
                            <div className="order__details__review__wrapper">
                                <Review id={order.review.id} text={order.review.text} ratingValue={order.review.rating} userFullName={order.review.userFullName} userImageUrl={order.review.userImageUrl} userId={order.review.userId} />
                            </div>
                        </td>
                    </tr>
                )
    ...
    const renderCustomerPlaceOrderButton = useCallback((order: IOrder) => {
